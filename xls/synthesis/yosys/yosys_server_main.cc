...
ABSL_FLAG(std::string, yosys_path, "yosys", "The path to the yosys binary.");
ABSL_FLAG(std::string, nextpnr_path, "", "The path to the nextpnr binary.");
ABSL_FLAG(std::string, synthesis_target, "ecp5",
...
ABSL_FLAG(std::string, sta_path, "sta", "The path to the sta binary.");
...
//       << "Valid --yosys_path must be provided";
  ...
  if (!synthesis_only) {
    if (!synthesis_target.empty()) {
      ...
      if (nextpnr_path.empty()) {
        nextpnr_path = std::string("nextpnr-") + synthesis_target;
      }
